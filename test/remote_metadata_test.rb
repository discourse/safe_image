...
module SafeImage
  class RemoteMetadataTest < TestCase
    ...
    def test_initial_headers_strip_credentials
      headers =
        Remote.initial_headers("Authorization" => "Bearer test-token", "Cookie" => "test-cookie", "Accept" => "image/*")
    ...
    def test_validate_uri_rejects_disallowed_port
      assert_raises(UnsafePathError) { Remote.validate_uri!(URI("https://example.com:81/x"), allow_private: false) }
    end
    ...
    def test_remote_animated_detection
      assert SafeImage.remote_animated?(
               server.url("/animated"),
               allow_private: true,
               max_bytes: 1_000_000,
    ...
    def test_remote_dominant_color
      assert_match(
        /\A\h{6}\z/,
        SafeImage.remote_dominant_color(
    ...
    def test_enforces_max_bytes
      assert_raises(LimitError) { SafeImage.remote_size(server.url("/huge.jpg"), allow_private: true, max_bytes: 10) }
    end
    ...
    def test_rejects_mismatched_signature_after_first_bytes
      assert_raises(InvalidImageError) { SafeImage.remote_size(server.url("/garbage.png"), allow_private: true) }
    ...
    def server
      @server ||=
        StubImageServer.new(
          ...
          "/slow-header.jpg" => {
            content_type: "image/jpeg",
            body: slow_header_jpg_body
          },
          "/garbage.png" => {
            content_type: "image/png",
            body: garbage_png_body
          },
          "/big-html-as-jpg.jpg" => {
            content_type: "text/html",
            body: big_html_body
