...
module SafeImage
  ...
  class SvgSemanticEqualityTest < TestCase
    ...
    def test_element_namespace_difference_is_unequal
      refute E.equal?(%(<svg xmlns="#{NS}"><rect/></svg>), %(<svg xmlns="#{NS}" xmlns:h="urn:h"><h:rect/></svg>))
    end

    def test_extra_namespace_declaration_is_unequal
      refute E.equal?(%(<svg xmlns="#{NS}"><rect/></svg>), %(<svg xmlns="#{NS}" xmlns:evil="urn:evil"><rect/></svg>))
    end

    def test_extra_child_is_unequal
      refute E.equal?(%(<svg xmlns="#{NS}"><rect/></svg>), %(<svg xmlns="#{NS}"><rect/><script/></svg>))
    end
    ...
    def test_text_content_difference_is_unequal
      refute E.equal?(%(<svg xmlns="#{NS}"><title>safe</title></svg>), %(<svg xmlns="#{NS}"><title>evil</title></svg>))
    end

    def test_text_presence_is_significant
      refute E.equal?(%(<svg xmlns="#{NS}"><title/></svg>), %(<svg xmlns="#{NS}"><title>x</title></svg>))
    end
  end
end
