...
module SafeImage
  ...
  class SandboxIntegrationTest < TestCase
    ...
    def test_svg_metadata
      svg = write_tmp("plain.svg", '<svg xmlns="http://www.w3.org/2000/svg" width="10" height="5"></svg>')

      assert_equal :svg, SafeImage.type(svg)
      assert_equal [10, 5], SafeImage.size(svg)
      assert_equal false, SafeImage.animated?(svg)
      assert_equal 1, SafeImage.orientation(svg)
    end
