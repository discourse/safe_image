...
module SafeImage
  ...
  class SandboxIntegrationTest < TestCase
    ...
    def test_sanitize_svg
      svg = write_tmp("bad.svg", %q{<svg xmlns="http://www.w3.org/2000/svg" width="1" height="1" onload="x"><script>x</script><rect width="1" height="1" onclick="x"/></svg>})
    ...
    # The namespace-prefix XSS (see SECURITY_FINDINGS) must not survive the
    # sandboxed svg-hush path: no executable handler reaches the output.
    def test_sanitize_svg_neutralises_namespace_prefixed_handler
      svg = write_tmp("xss.svg", <<~SVG)
      ...
      refute_match(/onload/i, File.read(svg), "event handler survived sanitisation")
    end
