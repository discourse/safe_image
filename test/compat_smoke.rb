...
Dir.mktmpdir do |dir|
  ...
  expected_outputs = [vips_thumb, im_thumb, crop, down, conv, webp]
  expected_outputs << heic if heic_supported
  raise "missing outputs" unless expected_outputs.all? { |f| File.file?(f) && File.size(f).positive? }
