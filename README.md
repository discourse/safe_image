...
#### `SafeImage.sanitize_svg!(path, max_pixels: nil)`

Sanitises an SVG in place by delegating to [svg-hush](https://github.com/cloudflare/svg-hush),
Cloudflare's memory-safe Rust SVG filter, which is bundled as a prebuilt
per-platform binary and run under the same argv-only + Landlock path as the other
external tools (no FFI). It strips scripting and event handlers, drops elements
and attributes outside its allowlist, and rewrites URLs to be same-origin only.
...
Before invoking svg-hush, the same byte-size cap, encoding rejection, and pixel
cap as the rest of the gem are enforced (so oversized/`DOCTYPE`/non-UTF-8 inputs
raise `LimitError`/`InvalidImageError` exactly as elsewhere). Malformed or
non-SVG input is rejected with `InvalidImageError`.

The output is safe to serve as an external resource: `<img src>`,
`background-image`, an `<object>`/`<iframe>`, or its own file.
...
The bundled binaries are produced by `script/build-svg-hush.sh` from a pinned
svg-hush version; an unbundled platform raises rather than compiling at install.
