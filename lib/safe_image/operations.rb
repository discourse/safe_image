        ...
        if use_jpegli_for_generated_jpeg?(format)
          with_temp_png(output) do |tmp_path|
            ...
            JpegliBackend.encode(
              ...
              chroma_subsampling:
                JpegliBackend.validate_chroma_subsampling!(chroma_subsampling, input_format: probe.input_format),
    ...
    def jpegli_convert_after_native_decode(input:, output:, quality:, max_pixels:, chroma_subsampling:)
      with_temp_png(output) do |tmp_path|
        ...
        JpegliBackend.encode(
          ...
          quality: quality,
          chroma_subsampling:
            JpegliBackend.validate_chroma_subsampling!(chroma_subsampling, input_format: decoded.fetch(:input_format)),
