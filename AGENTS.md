...
- Single file: `bundle exec rake test TEST=test/svg_hush_test.rb`
...
- SVG sanitising delegates to the bundled `svg-hush` binary (`SafeImage::SvgHush`), run through `Runner` (argv-only, Landlock when configured). `SafeImage::SvgMetadata` still enforces the byte/encoding/pixel pre-checks before the binary runs. `sanitize_svg!(path, max_pixels:)` produces document-safe output (safe to serve as an `<img>`/CSS-url/file). Binaries are prebuilt by `script/build-svg-hush.sh` from a pinned version and vendored under `vendor/svg-hush/`; never compile at install. To change filtering behaviour, bump the pinned svg-hush version and rebuild — do not reimplement it in Ruby.
