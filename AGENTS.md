...
- SVG metadata probing is bounded and non-rendering: keep the byte/depth/element/attribute caps, unsafe-encoding rejection, PI rejection, and root-dimension pixel cap before any parser result is trusted. A `DOCTYPE` is allowed only without an internal subset, because entities can only be declared in `[ ... ]` and that is the whole XXE/expansion surface. The byte scan enforcing that must keep skipping quoted literals, checking every occurrence, and failing closed on an unterminated declaration. This Nokogiri/libxml2 path is not Landlock-contained by Safe Image; do not claim it is.
