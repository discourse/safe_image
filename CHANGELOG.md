...
- **A `DOCTYPE` without an internal subset is now accepted.** The rule was a
  ...
  rejected. The byte scan enforcing it skips quoted literals, so a `>` or `[`
  inside a system literal cannot disguise the brackets; it checks every
  occurrence so a bare decoy cannot shield a later payload, and fails closed on
  an unterminated declaration. External DTDs remain unfetched and external
  entities unresolved.
